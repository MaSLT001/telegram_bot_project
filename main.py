...
random_range = (0, len(movies)-1) if movies else (0, 0)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привіт! Введи код фільму, щоб отримати інформацію або натисни кнопку нижче, щоб отримати рандомний фільм.",
        reply_markup=keyboard
    )

async def find_movie(update: Update, context: ContextTypes.DEFAULT_TYPE, code=None):
    code = code or update.message.text.strip()
    if code in movies:
        film = movies[code]
        text = f"🎬 *{film['title']}*\n\n{film['desc']}\n\n🔗 {film['link']}"
        await update.message.reply_text(
...
async def random_film_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    min_idx, max_idx = random_range
    max_idx = min(max_idx, len(keys)-1)
    selected_code = keys[random.randint(min_idx, max_idx)]
    await find_movie(update=query, context=context, code=selected_code)
...
async def set_range(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if min_val < 0 or max_val >= len(movies) or min_val > max_val:
            raise ValueError
        random_range = (min_val, max_val)
        await update.message.reply_text(f"✅ Діапазон рандомних фільмів встановлено: {min_val} - {max_val}")
    except:
        await update.message.reply_text("Використання: /range min max (числа від 0 до кількості фільмів-1)")
...
async def reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("reply_"):
        ...
        await random_film_callback(update, context)
...
if __name__ == "__main__":
    ...
    app.add_handler(CallbackQueryHandler(reply_callback, pattern=r"^(reply_|random_film)$"))
