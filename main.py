...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sorted_users = sorted(user_stats.items(), key=lambda x: x[1].get("visits", 0), reverse=True)
    top_users = "\n".join(
        [f"{i+1}. {data['name']} — {data['visits']} входів" for i, (uid, data) in enumerate(sorted_users[:5])]
    )

    text = (
        f"📊 *Статистика бота:*\n"
        f"👥 Користувачів: {total_users}\n\n"
        f"🏆 Топ-5 активних:\n{top_users if top_users else '—'}"
    )

    await update.message.reply_text(text, parse_mode="Markdown")

    # Надсилаємо сам файл stats.json
    if os.path.exists(STATS_FILE):
        await update.message.reply_document(document=open(STATS_FILE, "rb"))
